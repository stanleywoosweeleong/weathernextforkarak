...
locations around Karak, Pahang. Multilingual interface
...
This release migrates Karak from the older forecast-only build onto
...
```
https://stanleywoosweeleong.github.io/weathernextforkarak/
```
...
|---|---|---|---|---|
| Borneo Spring | 婆罗青泉 | 3.49200, 102.02400 | sheltered basin | durian |
| Kg. Sungai Dua | 甘榜双溪都亚 | 3.45972, 102.00000 | riverine | durian |
| Kg. Cinta Manis | 甘甜新村 | 3.45300, 102.03100 | sheltered basin | durian |
| Kg. Troh | 德乐新村 | 3.41194, 102.07889 | open plain | durian |
| The Valley | 加叻幽谷 | 3.38389, 102.02778 | sheltered basin | durian |
| 100% ORGANIC DURI RIN | 山本绫有机农场 | 3.45227, 101.99607 | sheltered basin | durian |
...
The app also seeds a default user display name (**加叻 / Karak**),
which stays editable via **Edit Name** in the app.

This build carries the seed version **`kk-arch1`** (the lowland-architecture
...
```
wnext-weathernextforkarak-202606041232
```
...
- **Storage namespace:** `weathernextforkarak__*` keys in `localStorage`, isolated
  from other WeatherNext regional builds so data never collides.
- **Cloud sync:** Firebase, namespaced under `appId: wnext-ag-v41-weathernextforkarak`.
...
The icon shows a couple in an orchard under a blue sky. The boot screen
background (sky-blue `#a0d9f5`) is matched to the icon's sky.
